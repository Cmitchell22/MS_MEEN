...
y(:,1) = y0(:,1) ;
fy(:,1) = f_temp ;
[y, fy] = Triangle(1, r-1, t, y, fy, zn_pred, zn_corr, N, PC, Probl ) ;

% Main process of computation by means of the FFT algorithm
ff = [0 2 ] ; nx0 = 0 ; ny0 = 0 ;
for q = 0 : Q
    L = 2^q ; 
    [y, fy] = DrawingBlocks(L, ff, r, Nr, nx0+L*r, ny0, t, y, fy, ...
...
function [y, fy] = DrawingBlocks(L, ff, r, Nr, nx0, ny0, t, y, fy, ...
                                  zn_pred, zn_corr, N , PC, Probl)
...
while ~stop
    ...
    stop = nxi+r-1 == nx0+L*r-1 | (nxi+r-1>=Nr-1) ; % Ci si ferma quando il Triangle attuale finisce alla fine del Square
    
    [zn_pred, zn_corr] = Square(nxi, nxf, nyi, nyf, fy, zn_pred, zn_corr, PC, Probl) ;
    
    [y, fy] = Triangle(nxi, nxi+r-1, t, y, fy, zn_pred, zn_corr, N, PC, Probl) ;
    i_Triangle = i_Triangle + 1 ;
    
    if ~stop
        if nxi+r-1 == nxf   % Il Triangle finisce dove finisce il Square -> si scende di livello
            i_Delta = ff(i_Triangle) ;
            Delta = i_Delta*r ;
            nxi = s_nxf(is)+1 ; nxf = s_nxf(is)  + Delta ;
            nyi = s_nxf(is) - Delta +1; nyf = s_nxf(is)  ;
            s_nxi(is) = nxi ; s_nxf(is) = nxf ; s_nyi(is) = nyi ; s_nyf(is) = nyf ;
        else % Il Triangle finisce prima del Square -> si fa un Square accanto
...
% =========================================================================
% =========================================================================
function [zn_pred, zn_corr] = Square(nxi, nxf, nyi, nyf, fy, zn_pred, zn_corr, PC, Probl)
...
% =========================================================================
% =========================================================================
function [y, fy] = Triangle(nxi, nxf, t, y, fy, zn_pred, zn_corr, N, PC, Probl)
